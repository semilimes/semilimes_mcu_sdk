...
/* Function: DcGauge.set
    ...
    Returns:
        void
*/
void DcGauge::set(char* title,int thickness,int degrees,int spacing,int fontSize,int value,char* displayValue,char* pointer,char* pointerColor,char* progressBar,char* progressBarColor)
{
    int size = headerSize+strlen(title)+json_data.intStrSize(thickness)+json_data.intStrSize(degrees)+json_data.intStrSize(spacing)+json_data.intStrSize(fontSize)+json_data.intStrSize(value)+strlen(displayValue)+strlen(pointer)+strlen(pointerColor)+strlen(progressBar)+strlen(progressBarColor)+1;//add '\0' for null-termination
    json = new char[size]; 
...
/* Function: DcGauge.addSegment
    ...
    Prototype:
        void DcGauge::addSegment(char* data);
